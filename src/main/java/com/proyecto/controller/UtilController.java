...
public class UtilController {
	...
	@GetMapping("/listaPaisPorNombreLike/{filtro}")
	@ResponseBody
	public ResponseEntity<List<Pais>> consulta(@PathVariable("filtro")String filtro){
		List<Pais> salida = null;
		try {
			if (filtro.equals("todos")) {
				salida = paisService.listaPaisPorNombreLike("%");
			}else {
				salida = paisService.listaPaisPorNombreLike("%"+filtro+"%");
			}
